...
class Compiler():
    ...
    def compile_ast(self, ast, var_dict, code=None):
        """
            ...
            an intermediate variable that holds the value of the sub
            expression. For example, the above requires adding these two
            statements to code:
                _x_1 = 2
                _y_1 = (_x_1 + 3)
            ...
            name x, while generation n > 0, is the name _x_n,
            (as in _x_1, _x_2, ...)

            Returns:

            A string s that is the Python expression that evaluates the
            expression in the ast, assuming that all the statements in code are
            evaluated prior to evaluating s.
        """
        if code is None:
            code = []
        ...
        if ntype == 'const':
            ...
            gen_num = var_dict.get(LHS, 0)
            var_dict[LHS] = gen_num + 1

            LHS = self.make_gen_name(LHS, gen_num)
            RHS = self.compile_ast(RHS)

            code_str = '{} = {}'.format(LHS, RHS)
            ...
            # NOT IMPLEMENTED
            if len(children) == 2:
                Expr = children[1].get_value()
                code_str = '({} {})'.format(op, Expr)
            elif len(children) == 3:
                (Expr1, Expr2) = children[1, 2]
                code_str = '({} {} {})'.format(Expr1, op, Expr2)

        else:

            # anything else, seriously wrong state.
            raise ValueError("Evaluation internal error, \
                             AST node type '{}' not implemented".
                             format(ntype))
