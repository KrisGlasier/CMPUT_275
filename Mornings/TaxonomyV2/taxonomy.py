...
def identify_thing(tree):
    ...
    "Does it have 4 legs?" Each internal node has two chioldren. Child 0
    ...
    form of...
    self.value = question
    self.children = answer. 0 = yes, 1 = no
    if no children, value is answer.
    """
    ...
    if len(Ch) is 2:
        ...
        (YesTree, NoTree) = Ch
        Answer = get_yes_no(question)
        if Answer:
            identify_thing(YesTree)
        else:
            identify_thing(NoTree)
        ...
        if Answer:
            ...
            new_question = get_nonblank_str(
                ...
                " and no means a '{}'".format(new_name, name))

            tree.set_value(new_question)
            tree.set_children([ValueTree(new_name),ValueTree(name)])
...
def do_things(tree):
    def get_things(tree):
        ...
        if len(Ch) is 0:
            ...
            return get_things(Ch[0]) + get_things(Ch[1])
...
def find_paths_to_thing(tree, name):
    ...
    if len(Ch) is 0:
        ...
        YesTree = Ch[0]
        NoTree = Ch[1]
        YesPaths = find_paths_to_thing(YesTree, name)
        NoPaths = find_paths_to_thing(NoTree, name)

        question = tree.get_value()
        py = list(map(lambda e: [[question, 'y']] + e, YesPaths))
        pn = list(map(lambda e: [[question, 'n']] + e, NoPaths))
        return py + pn
